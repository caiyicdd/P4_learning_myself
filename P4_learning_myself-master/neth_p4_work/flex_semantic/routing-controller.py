...
class RoutingController(object):
    ...
    def __init__(self):
        ...
        # bias[0]--bias[4] : bw / delay / loss / cost / header-lenth / reserve1 / reserve2 / reserve3
        self.bias = [1,0,0,0,1,0,0,0]
        self.init()
    ...
    def tunnel(self, sw1, sw2, src_type, dstip):
        ...
        cost = 0
        if src_type == "0x0800":
            # average payload length = 64byte , so 64/(64+20) ,20 is ipv6 header length
            header_length = 0.76
            # average payload length = 256byte , so 256/(256+20) ,20 is ipv6 header length
            header_length = 0.93
        elif src_type == "0x08dd":
            # average payload length = 64byte , so 64/(64+40) ,40 is ipv6 header length
            header_length = 0.62
            # average payload length = 256byte , so 256/(256+20) ,20 is ipv6 header length
            header_length = 0.86

        res_wight = bw1 * self.bias[0] * header_length * self.bias[4] + delay1 * self.bias[1] + loss1 * self.bias[2] + cost * self.bias[3]
        res_type = src_type
        src_gre_ip = None
        dst_gre_ip = None
        ...
        for i in range(len(self.fullRouting_tbl[sw1])):
            ...
            head_len = header_length
            if self.fullRouting_tbl[sw1][i][0][0] == "0x0800" and self.fullRouting_tbl[sw1][i][0][0] != src_type:
                # average payload length = 64byte , so 64/(64+40+16+20) ,20 is ipv6 header length,16 is gre header length , 40 is ipv6 header length
                head_len = 0.46
                # average payload length = 256byte , so 256/(256+40+16+20) ,20 is ipv6 header length,16 is gre header length , 40 is ipv6 header length
                head_len = 0.77
            elif self.fullRouting_tbl[sw1][i][0][0] == "0x08dd" and self.fullRouting_tbl[sw1][i][0][0] != src_type:
                # average payload length = 64byte , so 64/(64+20+16+40) ,20 is ipv6 header length,16 is gre header length , 40 is ipv6 header length
                head_len = 0.46
                # average payload length = 256byte , so 256/(256+40+16+20) ,20 is ipv6 header length,16 is gre header length , 40 is ipv6 header length
                head_len = 0.77

            wight2 = bw2 * self.bias[0] * head_len * self.bias[4] + delay2 * self.bias[1] + loss2 * self.bias[2] + cost * self.bias[3]
